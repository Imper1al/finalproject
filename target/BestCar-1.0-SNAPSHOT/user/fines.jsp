...
<head>
    <meta charset="utf-8">
    <title><fmt:message key="user.fines.title" bundle="${bundle}"/></title>
...
<div class="cover-container d-flex w-100 h-100 p-3 mx-auto flex-column d-flex h-100 text-center text-dark">
    ...
    <main>
        <h1 class="mb-5"><fmt:message key="user.fines.main" bundle="${bundle}"/></h1>
        <h3>${requestScope.message}</h3>
        <table class="table table-striped table-sm">
            <tr>
                <th><fmt:message key="user.fines.car" bundle="${bundle}"/></th>
                <th><fmt:message key="user.fines.damage" bundle="${bundle}"/></th>
                <th><fmt:message key="user.fines.description" bundle="${bundle}"/></th>
                <th><fmt:message key="user.fines.price" bundle="${bundle}"/></th>
                <th><fmt:message key="user.fines.pay" bundle="${bundle}"/></th>
            </tr>
            <c:forEach var="fine" items="${requestScope.fines}">
                <tr>
                    <td>${fine.carId}</td>
                    <td>${fine.damage}</td>
                    <td>${fine.damageDesc}</td>
                    <td>${fine.sum}</td>
                    <td>
                        <form class="mb-auto" method="post" action="pay_fine">
                            <input type="text" hidden name="id" value="${fine.id}" />
                            <input type="text" hidden name="userId" value="${fine.userId}" />
                            <input type="text" hidden name="price" value="${fine.sum}" />
                            <input type="text" hidden name="historyId" value="${fine.historyId}" />
                            <button class="btn btn-outline-secondary text-dark margin" type="submit"><fmt:message key="user.fines.pay.button" bundle="${bundle}"/></button>
                        </form>
                    </td>
                </tr>
            </c:forEach>
